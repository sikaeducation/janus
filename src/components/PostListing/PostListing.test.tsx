...
test.skip(
	...
	/>);
		...
		expect(slug)
			.toHaveTextContent("some-post");

		const fullLabel = screen.getByTestId("full-label");
		expect(fullLabel)
			.toHaveTextContent("Full Label");

		const shortLabel = screen.getByTestId("short-label");
		expect(shortLabel)
			.toHaveTextContent("Short Label");

		const tinyLabel = screen.getByTestId("tiny-label");
		expect(tinyLabel)
			.toHaveTextContent("Tiny Label");
	},
);

test.skip(
	"<PostListing /> calls mouse handlers on click",
	() => {
		mockUseClipboard.mockReturnValue([
			false,
			jest.fn(),
...
test.skip(
	"<PostListing /> calls keyboard handlers on enter",
	async() => {
		mockUseClipboard.mockReturnValue([
			false,
			jest.fn(),
	...
	/>);
		const postListing = screen.getByTestId("PostListing");
		tab();
		await expect(postListing)
			.toHaveFocus();
		keyboard("{enter}");
		expect(keyboardHandler)
			.toHaveBeenCalled();
	},
);

test.skip(
	"<PostListing /> copies to clipboard",
	() => {
		...
		mockUseClipboard.mockReturnValueOnce([
			false,
			clipboardSpy,
		]);

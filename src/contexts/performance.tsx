...
const addPostSlugToQuestionPayload = map((performance: evaluatedQuestionPerformance) => ({
	...performance,
	originalPostSlug: performance.payload.originalPostSlug,
}));
const lastPerformanceBySlugByQuestion = mapValues(maxBy((performance: postedPerformance) => Date.parse(performance.createdAt)));
...
export function PerformanceProvider({ children }: props){
  ...
  >([]);
	...
	useSocketHandlers({
		...
		"new-performance": (performance: evaluatedSubmissionPerformance) => setPerformances((previous) => [
			...previous,
			performance,
		]),
		"new-performance-notice": (performance: postedPerformance) => setToasts([
			...toasts,
			performance.userId,
		]),
		"new-evaluation": (evaluation: postedEvaluation) => setEvaluations((previous) => [
			...previous,
			evaluation,
		]),
		"new-evaluation-notice": (evaluation: postedEvaluation) => setToasts([
			...toasts,
			evaluation.status,
		]),
	});
	...
	const lastQuestionPerformancesBySlug = flow([
		...
		mapValues(mapValues(maxBy((performance: postedPerformance) => Date.parse(performance.createdAt)))),
	...
	const lastQuestionPerformancesBySlugByLearnerByQuestion = flow([
		mapValues(flow([
			...
			mapValues(flow([
				groupByPost,
				lastPerformanceBySlugByQuestion,
			])),
		])),
	])(performancesByQuestion);

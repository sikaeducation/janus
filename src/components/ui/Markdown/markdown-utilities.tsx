...
export function formatLinks({
  ...
  return function _formatLinks({
    ...
    const lastNonQuestionPerformance = slug
      && user?.email
      && lastPerformanceBySlugByLearner[slug]?.[user?.email || ''];

    const lastQuestionPerformances = lastQuestionPerformancesBySlugByLearnerByQuestion[slug]?.[
      user?.email || ''
    ];
    const lastPerformance = post?.type === 'questions' && lastQuestionPerformances
      ? {
        type: 'questions',
        postSlug: slug,
        userId: user?.email || '',

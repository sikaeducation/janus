...
async function mockApiResponse() {
  ...
  await page.route("**/programs/*", route => {
    route.fulfill({
      ...
      body: JSON.stringify({program: getSampleProgram()}),
    })
  })
...
describe("need to rework these", () => {
  test("unit navigation", async () => {
    const {page, browser} = await mockApiResponse()
  ...
  test("section navigation", async () => {
    ...
    await page.goto("http://frontend:3000/websites")
  ...
  test("activity navigation", async () => {
    const {page, browser} = await mockApiResponse()
    await page.goto("http://frontend:3000/websites/websites-1/html-div-span")
